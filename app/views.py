    ...
    def post(self,request):
        ...
        if form1.is_valid() & form2.is_valid():
            ...
            try:
                ...
                messages.success(request, 'Congradulations!!, Volunteer Profile created Successfully')
            except:
                messages.warning(request, 'Profile Not Created')
        return render(request, "signup_volunteer.html", locals())
...
def index_admin(request):
    ...
    totaldonars = Donor.objects.all().count()
    totalvolunteers = Volunteer.objects.all().count()
    totalpendingdonations = Donation.objects.filter(status = "pending").count()
    totalaccepteddonation = Donation.objects.filter(status = "accept").count()
    totaldeliverddonation = Donation.objects.filter(status= "Donation Delivered Successfully").count()
...
class accepted_donationdetail(View):
    ...
    def post(self, request, pid):
        ...
        try:
            ...
            messages.success(request, 'Failed To Allocate Volunteer')
        return render(request, "accepted-donationdetail.html",locals())
...
class view_donationdetail(View):
    ...
    def post(self,request,pid):
        ...
        adminremark = request.POST['adminremark']
        #print("status = ",status,"remarks = ",adminremark)
        try:
            donation.adminremark = adminremark
...
def index_volunteer(request):
    ...
    totalDonationDelivered = Donation.objects.filter(volunteer=volunteer, status="Donation Deliverd Successfully").count()
